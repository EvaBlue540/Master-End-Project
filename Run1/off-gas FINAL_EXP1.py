...
# ============================================================
path4 = "path_to_luke4_offgas.csv"  # Replace with actual path to Luke 4 off-gas data
path5 = "path_to_luke5_offgas.csv"  # Replace with actual path to Luke 5 off-gas data  


# Experiments: (name, path, C_acetate in mCmol, cycle_code)
EXPERIMENTS = [
    ("Luke4", path4, 176, "CN40"),
    ("Luke5", path5, 88, "CN20"),
]
...
selected_cycles = [13,17,25,27,29,31,39,54,61,71]
...
def process_experiment(experiment_name, csv_path, C_acetate, start_date, end_date, 
    ...
    for i in range(0, num_intervals - 1, 2):
        ...
        if (cycle_data_quality == 'COMPLETE' or (is_special_case and has_valid_data)) and has_valid_data:
            ...
            # - Different sensitivity for Luke 5 vs Luke 6
            min_feast_time = 2.0
            max_feast_time = 6.5
            
            # Experiment-specific parameters for gradient-based detection
            if experiment_name == "Luke4":
                gradient_threshold = -10  # Very sensitive for Luke4 (catch early onset of decline)
                min_sustained_points = 3  # Require 4 consecutive points below threshold
            else:  # Luke5
                gradient_threshold = -10  # Steep slope required for Luke5 (avoid noise-induced early detection)
                min_sustained_points = 3  # Require 4 consecutive points (30 min with 15-min intervals)
...
# ============================================================
    ...
    out_xlsx = f"Excel output/PHA_SBR_results_{experiment_name}_EXP1.xlsx"
    ...
    for cycle_count, phase_idx in enumerate(c_phase_indices):
        ...
        if not feast_row.empty:
            ...
            if len(feast_times) > 0:
                ...
                ax.axvline(feast_end, linestyle='--', color='#457B9D', linewidth=1.8, alpha=0.8)
                # Add effluent marker line using cycle end date
                c_phase_start_hour = pd.to_datetime(feast_start).hour
                c_phase_end_date = pd.to_datetime(c_phase_end).date()
                if c_phase_start_hour == 9:  # Day C-phase: effluent at 15:30 same day
                    effluent_time = pd.to_datetime(f"{c_phase_end_date} 15:30")
                else:  # Night C-phase: effluent at 03:30 next day
                    effluent_time = pd.to_datetime(f"{c_phase_end_date} 03:30")
                ax.axvline(effluent_time, linestyle='--', color='gray', linewidth=1.5, alpha=0.5)
                # Annotate effluent
                y_lim = ax.get_ylim()
                ax.annotate('Effluent',
                           xy=(effluent_time, y_lim[1] * 0.8), xytext=(0, 0),
                           textcoords='offset points', fontsize=9, fontweight='normal',
                           bbox=dict(boxstyle='round,pad=0.3', facecolor='#F0F0F0', alpha=0.7, edgecolor='gray', linewidth=1),
                ...
                base_date = pd.to_datetime(feast_start).date()
                if cycle_count == 1:
                ...
                ax.annotate(f'{feast_hours:.1f}h\nY$_{{PHA/Ac}}$={pha_yield:.2f}',
                           xy=(annotation_time, y_lim[1] * 0.5), xytext=(0, 0),
    ...
    plot_filename = f"plots_EXP1/{experiment_name}_feastphase_EXP1.png"
    plt.tight_layout()
...
# ============================================================
# MAIN EXECUTION: Process both experiments in one run
# ============================================================
if __name__ == "__main__":
    ...
    print(f"Data range: {start_date.date()} to {end_date.date()}")
    print(f"Luke 4: C_acetate = 176 mCmol")
    print(f"Luke 5: C_acetate = 88 mCmol")
    print(f"{'='*60}")
